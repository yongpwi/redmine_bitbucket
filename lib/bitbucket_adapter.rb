...
class BitbucketAdapter
  ...
  def initialize(json)
    ...
    when 'git'
      @scm = BitbucketGit
      @cmd = Setting.plugin_redmine_bitbucket[:git_cmd_path].empty? ? 'git' : Setting.plugin_redmine_bitbucket[:git_cmd_path]
    when 'hg'
      @scm = BitbucketHg
      @cmd = Setting.plugin_redmine_bitbucket[:hg_cmd_path].empty? ? 'hg' : Setting.plugin_redmine_bitbucket[:hg_cmd_path]
    else
