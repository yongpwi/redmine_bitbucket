...
2. Connecting Bitbucket to Redmine
    * Go to the repository Admin interface on Bitbucket.
    * Under "Services" add a new "POST" of the format: "[redmine_installation_url]/hooks/bitbucket/:project_id(&key=[***])"  (for example "http://example.com/hooks/bitbucket/example_project&key=supersecret").
    * Note: since v1.0, the POST url has changed to hooks/bitbucket/:project_id, which is not backwards compatible.
